...
| `could not translate host name "db" to address` (Linux more than Windows) | Often **(1)** `.env` was saved with **Windows CRLF**; inside a Linux container the database host can end up wrong until line endings are normalized—**save `.env` as LF** (`dos2unix .env`, or your editor “LF”). **(2)** Compose services not on the same **network** or Postgres service not named **`db`**. **(3)** Rare: Docker **embedded DNS** not ready; the API now **retries** DB connect for ~2 minutes on startup. |
| `getaddrinfo EAI_AGAIN api` from **web** / Vite proxy | The **API container is not running or not yet healthy**, so hostname **`api`** does not resolve from `web`. Check `docker compose ps` and `api` logs. This compose file starts **`web` only after `api` is healthy** so Vite does not proxy before the API listens. |
